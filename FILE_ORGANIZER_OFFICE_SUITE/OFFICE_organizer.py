# --- START OF MODIFIED FILE OFFICE_organizer_blue_themed.py ---
...
# Blue Theme Colors (Adapted for Office)
BLUE_COLORS = {
    "background": "#1B2333",       # Consistent deep blue background (From Personal)
    "secondary_bg": "#1B2333",    # Use the same for all backgrounds (From Personal)
    "primary": "#3B82F6",         # Vibrant blue for primary actions (From Original Office)
    "secondary": "#2563EB",       # Darker blue for secondary actions (From Original Office)
    "accent": "#60A5FA",          # Lighter blue for accents (e.g., Browse buttons) (From Original Office)
    "text_primary": "#FFFFFF",     # White text for high contrast (From Personal/Office)
    "text_secondary": "#94A3B8",  # Light blue/gray for secondary text (From Original Office)
    "border": "#334155",          # Slightly lighter border than background (Adapted)
    "input_bg": "#334155",        # Input background distinct from main bg (Adapted from Office)
    "success": "#34D399",          # Green (From Original Office)
    "warning": "#FBBF24",          # Yellow (From Original Office)
    "danger": "#F87171",           # Red (From Original Office)
    "progress_bar_bg": "#2563EB", # Background of the progress bar area (matches secondary button) (From Original Office)
    "progress_bar_chunk": "#60A5FA", # Bright blue chunk (From Original Office)
}

FONT_FAMILY = "Segoe UI"      # Consistent font

class OfficeFileOrganizerApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Time to Organize Your Office!")

        # --- Use the Blue Theme ---
        self.current_colors = BLUE_COLORS

        # --- Office File Categories (From Original Office) ---
        self.file_categories = {
            "Documents": [".doc", ".docx", ".pdf", ".txt", ".rtf", ".odt", ".pages"],
            "Spreadsheets": [".xls", ".xlsx", ".csv", ".ods", ".numbers"],
            "Presentations": [".ppt", ".pptx", ".key", ".odp"],
            "Images": [".jpg", ".jpeg", ".png", ".bmp", ".tiff", ".gif", ".webp", ".heic"],
            "PDFs": [".pdf"], # Note: Overlaps with Documents, PDF will likely take precedence if listed first
            "Email Files": [".eml", ".msg", ".vcf", ".ics"],
            "Archives": [".zip", ".rar", ".7z", ".tar", ".gz"],
            "Database Files": [".accdb", ".mdb", ".db", ".dbf", ".sql", ".sqlite"],
            "Project Files": [".mpp", ".mpx", ".xer", ".planner", ".gan"],
            "Text & Markdown": [".txt", ".md", ".markdown", ".rtf"], # Note: .txt, .rtf overlap with Documents
            "Web Files": [".html", ".htm", ".css", ".js", ".json", ".xml"],
            "Financial": [".qbw", ".qbb", ".tax", ".ifx", ".ofx"],
            "Source Code": [".py", ".java", ".cpp", ".c", ".h", ".js", ".php", ".html", ".css", ".sql"] # Note: Overlaps Web Files
            # Consider consolidating/refining categories to avoid significant overlap if needed
        }
        ...
        # --- Window Size (Adapted from Personal, slightly adjusted) ---
        screen = QGuiApplication.primaryScreen().availableGeometry()
        max_width, max_height = screen.width(), screen.height()
        self.setGeometry(100, 100, min(900, max_width - 100), min(650, max_height - 100)) # Slightly adjusted default
        self.setMinimumSize(650, 500) # Adjusted minimum
        self.setMaximumSize(max_width, max_height)

        self.setup_ui()
        self.update_stylesheet() # Apply the blue theme stylesheet

    def setup_ui(self):
        # Structure largely taken from PERSONAL_organizer_purple.py
        central_widget = QWidget(self)
        self.setCentralWidget(central_widget)
        main_layout = QVBoxLayout(central_widget)
        main_layout.setContentsMargins(25, 20, 25, 20) # Spacing from Personal
        main_layout.setSpacing(15) # Adjusted spacing

        # --- Header (Style from Personal) ---
        ...
        self.greeting_label = QLabel(self.get_greeting()) # Use Office greeting
        ...
        # App title (Right)
        app_title = QLabel("Office File Organizer") # Updated title
        ...
        # --- Input Section (Layout from Personal, Placeholders from Office) ---
        ...
        source_label = QLabel("Source Folder:") # Text consistent with Personal
        source_label.setFont(QFont(FONT_FAMILY, 10, QFont.Weight.Bold))
        source_layout.addWidget(source_label)
        self.source_entry = QLineEdit()
        self.source_entry.setPlaceholderText("Where are the files you want to organize?...") # Placeholder from Office
        ...
        dest_label = QLabel("Destination Folder:") # Text consistent with Personal
        dest_label.setFont(QFont(FONT_FAMILY, 10, QFont.Weight.Bold))
        dest_layout.addWidget(dest_label)
        self.dest_entry = QLineEdit()
        self.dest_entry.setPlaceholderText("Where do you want to organize your files?...") # Placeholder from Office
        ...
        # --- File Type Selection (Layout/Functionality from Personal, Categories from Office) ---
        ...
        file_type_label = QLabel("Select file types to organize:") # Text from Office
        ...
        select_all_btn = QPushButton("Select All") # Added from Personal
        ...
        deselect_all_btn = QPushButton("Deselect All") # Added from Personal
        ...
        checkbox_scroll.setMaximumHeight(180) # Max height from Personal
        self.file_types = list(self.file_categories.keys()) # Use Office categories
        self.check_vars = {}
        ...
        num_cols = 3 # Adjusted columns for potentially fewer Office categories
        ...
        # --- Progress and Status (Layout/Functionality from Personal) ---
        ...
        # --- Action Buttons (Layout from refactor, Text from Office) ---
        ...
        footer = QLabel(f"© {datetime.now().year} Office File Organizer")
        ...
        # --- Timer for Time/Greeting Update (From Personal) ---
    ...
    # --- open_custom_style_dialog, save_custom_names remain the same as Personal ---
    ...
    def open_custom_style_dialog(self):
        # Dialog inherits main window stylesheet (structure from Personal)
        dialog = QDialog(self)
        dialog.setWindowTitle("Customize Folder Names")
        dialog.setMinimumWidth(500)
        dialog.setMinimumHeight(400) # Adjust height if needed for office categories
        ...
        # Use the current (sorted) Office categories
    ...
    # --- as Personal, but use blue theme colors via update_status ---
    ...
    def get_current_time(self):
        # Format consistent with original screenshot
        return datetime.now().strftime("%I:%M:%S %p")

    def get_greeting(self):
        # Greetings from original Office script
        hour = datetime.now().hour
        if hour < 12:
            return "Good morning, ready to organize your files?"
        elif hour < 18:
            # Greeting from screenshot
            return "Good afternoon! Let's get those files sorted."
        else:
            return "Good evening, time to reorganize your files."

    def update_time_and_greeting(self):
        # From Personal
        self.time_label.setText(self.get_current_time())
        current_greeting = self.get_greeting()
        # Update greeting only if it changes to avoid unnecessary redraws
        if self.greeting_label.text() != current_greeting:
            self.greeting_label.setText(current_greeting)

    def closeEvent(self, event):
        # From Personal (Handles closing during organization)
        if self.is_organizing:
            ...
            if reply == QMessageBox.StandardButton.Yes:
                self.is_organizing = False # Signal thread to stop (if checked)
                # Give thread a moment to potentially react if it checks self.is_organizing
                # Although the current loop doesn't check mid-operation, this is good practice
                time.sleep(0.1)
                event.accept()
            else:
                event.ignore()
    ...
    def update_stylesheet(self):
        # Stylesheet structure from Personal, using BLUE_COLORS
        sheet = f"""
            QMainWindow, QDialog {{
            ...
            QLabel {{
                ...
                color: {self.current_colors['text_primary']}; /* Default: White text */
            }}
            QLabel#TimeLabel {{ /* White, already default */ }}
            QLabel#GreetingLabel {{ color: {self.current_colors['text_secondary']}; }}
            QLabel#app_title {{ color: {self.current_colors['text_primary']}; }} /* White title */
            QLabel#StatusLabel {{ color: {self.current_colors['text_secondary']}; }} /* Default status color */
            QLabel#FooterLabel {{ color: #64748B; }} /* Specific color for footer like original Office*/
            QDialog QLabel {{ padding-top: 2px; }} /* Spacing in dialog */

            QLineEdit {{
                background-color: {self.current_colors['input_bg']}; /* Use input bg */
            ...
            QLineEdit:focus {{
                border: 1px solid {self.current_colors['primary']}; /* Highlight with primary blue */
            }}

            QPushButton {{
                border-radius: 5px;
                padding: 9px 18px; /* Standard padding */
                font-size: 10pt;
                font-weight: 500;
                border: none;
            ...
            QPushButton:disabled {{
                background-color: {self.current_colors['secondary']}; /* Use secondary blue */
                opacity: 0.6;
            }}

            /* Specific Button Styles using Object Names */
            QPushButton#PrimaryButton {{ background-color: {self.current_colors['primary']}; }} /* Primary Blue */
            QPushButton#SecondaryButton {{ background-color: {self.current_colors['secondary']}; }} /* Secondary Blue */
            QPushButton#AccentButton {{ background-color: {self.current_colors['accent']}; }} /* Accent Blue */

            /* Text Button Style for Select/Deselect All */
            QPushButton#TextButton {{
                 background-color: transparent;
                 color: {self.current_colors['accent']}; /* Use accent blue */
                 font-size: 9pt;
                 font-weight: normal;
                 padding: 4px 8px;
            ...
            QProgressBar {{
                border: none;
                border-radius: 5px;
                background-color: {self.current_colors['progress_bar_bg']}; /* Use blue progress bg */
                height: 10px;
                text-align: center;
                color: transparent; /* Hide percentage text */
            }}
            QProgressBar::chunk {{
                background-color: {self.current_colors['progress_bar_chunk']}; /* Use blue chunk color */
                border-radius: 5px;
            }}
            ...
            QCheckBox::indicator:checked {{
                background-color: {self.current_colors['primary']}; /* Use primary blue */
                border: 1px solid {self.current_colors['primary']};
                /* Optional: Add a white checkmark SVG or image */
                /* image: url(path/to/white_check.svg); */
            }}
            QCheckBox::indicator:checked:hover {{ opacity: 0.85; }}

            QScrollArea {{ border: none; background-color: transparent; }}
            /* Target inner widget specifically for transparent background */
            QScrollArea > QWidget > QWidget {{ background-color: transparent; }}
            QScrollArea#DialogScrollArea {{ /* Style for dialog scroll if needed */
                /* border: 1px solid {self.current_colors['border']}; */
            }}
            ...
            QScrollBar::handle:vertical {{
                background: {self.current_colors['primary']}; /* Use primary blue */
                min-height: 25px; border-radius: 5px;
            }}
            QScrollBar::handle:vertical:hover {{ background: {self.current_colors['accent']}; }} /* Use accent blue */
            ...
            /* QMessageBox Styling */
            QMessageBox {{ background-color: {self.current_colors['secondary_bg']}; }} /* Use secondary bg */
            QMessageBox QLabel {{ color: {self.current_colors['text_primary']}; font-size: 10pt; }}
            QMessageBox QPushButton {{
                 min-width: 80px;
                 background-color: {self.current_colors['primary']}; /* Primary blue for buttons */
                 color: {self.current_colors['text_primary']};
            }}
    ...
    def update_ui_colors(self):
        ...
        # Only set colors not easily targeted or needing explicit initial set
        ...
        if footer:
            footer.setStyleSheet("color: #64748B;") # Specific footer color

    def show_about_dialog(self):
        about_text = (
            "<h2>Office File Organizer</h2>"
            f"<p>Version 1.0.0 • © {datetime.now().year} JUST ELSON PRODUCTIVE.labs</p>"
            "<p>A professional tool for organizing your office documents and files.</p>"
            "<p>Features:</p>"
            "<ul>"
        ...
        QMessageBox.about(self, "About Office File Organizer", about_text)


if __name__ == "__main__":
    app = QApplication(sys.argv)
    # app.setStyle("Fusion") # Fusion often helps consistency

    # --- Application Icon (Use Office Icon) ---
    # Assume icon is in 'app_icons' subdir relative to script
    icon_dir = os.path.dirname(__file__) # Get directory of the script
    icon_path = os.path.join(icon_dir, "app_icons", "icon_office.png") # Construct path
    ...
    window = OfficeFileOrganizerApp()
    # Standard window flags (like Personal)
...
# --- END OF MODIFIED FILE OFFICE_organizer_blue_themed.py ---